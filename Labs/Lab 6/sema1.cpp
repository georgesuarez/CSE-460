...
int main(int argc, char *argv[])
{
    ...
    if (strcmp(argv[1], "1") == 0)
    {
        for (i = 0; i < 10; i++)
        {
            if (!SEM_DOWN())
                exit(EXIT_FAILURE);
            cout << ce;
            fflush(stdout);          //entering critical section
            pause_time = rand() % 3; //simulate critical section
            sleep(pause_time);
            cout << cl;
            fflush(stdout); //leaving critical section
            if (!SEM_UP())
                exit(EXIT_FAILURE); //signal other waiting process
            pause_time = rand() % 2;
            sleep(pause_time);
        }
        cout << endl
             << getpid() << " finished!" << endl;
        if (argc > 0)
        {
            sleep(2);
            del_semvalue();
    ...
    {
        cout << "I am going to wait forevr..." << endl;
        while (1)
        {
        }
    }
